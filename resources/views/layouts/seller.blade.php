...
<body x-data="{ gray: false }">
    <div class="relative">
        <x-seller-navbar />
        <div x-show="gray" :class="gray ? 'bg-gray-800/75 absolute inset-0 w-full z-40 h-full' : ''" x-transition>
        </div>
        @if (session()->has('success'))
        <div class="bg-emerald-50 rounded-md p-2 mt-[4.5rem]">
            <div class="flex justify-between max-w-6xl mx-auto">
                <p class="text-green-800 font-medium text-sm">{{session()->get('success')}}</p>
                <span class="material-icons text-sm text-emerald-700">close</span>
            </div>
        </div>
        @endif
        <main class="mx-auto max-w-6xl px-4 lg:px-0 my-10 pt-[2rem] md:pt-10 lg:pt-12">
            @yield('content')
        </main>
        <hr>
        <footer>
